...
from MathModularFrame import MathModularEnv
from utils import _get_epi_num, _get_group_id, _get_user_data, _get_user_id, _get_group_users, _get_user2group_dict
...
def train(config):
    ...
    if os.path.exists(log_dir) == False:
        os.mkdir(log_dir)
    env_train = MathModularEnv(config)
...
def test(config):
    env_test = MathModularEnv(config)
    ...
    agent_answer, user_answer, agent_resptime, user_resptime, est_answer, est_resptime, fail_rate, trajectory_list = [], [], [], [], [], [], [], [] 
    while trial_count < config['test_epi_num']:
        old_obs = obs
        action, _states = model.predict(obs, deterministic=False)
        # print('action: ', action)
        if config['fine_level'] == 'discrete':
            trajectory_piece.append(action[0])
        else:
            trajectory_piece.append(action[0][0])
        ...
        if dones[0] == True:
            ...
            if fail_flag == 0:
                ...
                agent_resptime.append(config['max_time_set'])
            ...
            trajectory_list.append(trajectory_piece)
            trajectory_piece = []
    ...
    if config['save_eval_file'] == True:
        ...
        save_list(trajectory_list, Test_MODEL_PATH+'/'+config['test_result_name']+'_trajectory.txt')
...
config = {
    ...
    'env_type': None,
    'train_step': 10, # 100000
    'test_epi_num': None,
    'test_format': None,
    'log_dir': None,
    'save_eval_file': True,
    'vis_eval': False,
    'fine_level': 'continuous', # 'discrete' or 'continuous'
}

def eval_user_each_simple(config,user_id):
    config['select_type'] = 'user'
    config['train_dataset_path'] = '../svm_model/timecare~25k_estimate_train_ind_user_encode_norm.csv'

    save_list([['user_id','test_mape','test_fail_rate']], 'rl_user_metrics.csv')
    config['log_dir'] = 'user_model/' + str(int(user_id)) + '/' + datetime.now().strftime('%y_%m_%d_%H_%M_%S') + '/'
    if os.path.exists('user_model/') == False:
        os.mkdir('user_model/')
    if os.path.exists('user_model/' + str(int(user_id))) == False:
        os.mkdir('user_model/' + str(int(user_id)))
    ...
    model_metrics_rl = [user_id] + test_metric[-2:]

    save_list([model_metrics_rl], 'rl_user_metrics.csv')

def eval_user_each(config, user_id):
    config['select_type'] = 'user'
    config['train_dataset_path'] = '../svm_model/timecare~25k_estimate_train_ind_user_encode_norm.csv'
...
def eval_group(config):
    ...
    save_list([['group_id','train_accu','train_f1','train_precision','train_recall','train_mae', 'train_mape','train_fail_rate','test_accu','test_f1','test_precision','test_recall','test_mae','test_mape','test_fail_rate']], 'rl_group_metrics.csv')
    save_list([['group_id','train_accu','train_f1','train_precision','train_recall','train_mae', 'train_mape','train_fail_rate','test_accu','test_f1','test_precision','test_recall','test_mae','test_mape','test_fail_rate']], 'svm_group_metrics.csv')
    save_list([new_head], 'timecare~25k_rl_estimate_train_group.csv')
    save_list([new_head], 'timecare~25k_rl_estimate_test_group.csv')
    for counter_i, group_name in enumerate(group_all):
        config['log_dir'] = 'group_model/' + group_name + '/' + datetime.now().strftime('%y_%m_%d_%H_%M_%S') + '/'
        if os.path.exists('group_model/') == False:
            os.mkdir('group_model/')
        if os.path.exists('group_model/' + group_name) == False:
            os.mkdir('group_model/' + group_name)
        ...
        model_metrics_rl = [group_name] + train_metric + test_metric
        model_metrics_svm = [group_name] + train_metric_svm + test_metric_svm

        save_list([model_metrics_rl], 'rl_group_metrics.csv')
        save_list([model_metrics_svm], 'svm_group_metrics.csv')
        ...
        save_list(dataset_train_piece_arr, 'timecare~25k_rl_estimate_train_group.csv')
        save_list(dataset_test_piece_arr, 'timecare~25k_rl_estimate_test_group.csv')
...
def eval_group_each(config, group_name):
    config['select_type'] = 'group'
    config['train_dataset_path'] = '../svm_model/timecare~25k_estimate_train_group_encode_norm.csv'
    train_dataset_all = pd.read_csv(config['train_dataset_path'])
    group_all = _get_group_id(train_dataset_all)
...
def eval_group_tune(config):
    ...
    save_list([['group_id','test_mape','test_fail_rate']], 'rl_group_metrics.csv')
    ...
    for counter_i, group_name in enumerate(group_all):
        ...
        model_metrics_rl = [group_name] + test_metric[-2:]
...
def eval_general_all(config):
    ...
    config['log_dir'] = 'general_model/' + datetime.now().strftime('%y_%m_%d_%H_%M_%S') + '/'
    if os.path.exists('general_model/') == False:
        os.mkdir('general_model/')
    config['env_type'] = 'train'
    config['dataset_all_path'] = config['train_dataset_path']
    train(config)
    ...
    estimated_dataset_train.to_csv('timecare~25k_rl_estimate_train_general.csv',index=False)
    estimated_dataset_test.to_csv('timecare~25k_rl_estimate_test_general.csv',index=False)
    model_metrics_all.to_csv('rl_general_metrics.csv',index=False)
    model_metrics_all_svm.to_csv('svm_general_metrics.csv',index=False)

def eval_general_tune(config):
    config['select_type'] = 'all'
    config['train_dataset_path'] = '../svm_model/timecare~25k_estimate_train_general_encode_norm.csv'
    train_dataset_all = pd.read_csv(config['train_dataset_path'])

    save_list([['general','test_mape','test_fail_rate']], 'rl_general_metrics.csv')
    ...
    model_metrics_rl = ['general'] + test_metric[-2:]
    save_list([model_metrics_rl], 'rl_general_metrics.csv')
...
def eval_leave_one_participant_out_group(config):
    ...
    for counter_i,group_name in enumerate(group_all):
        ...
        for counter_j,user_id in enumerate(user_all_per_group):
            config['log_dir'] = 'lopo_model/' + group_name + '/' + str(int(user_id)) + '/' + datetime.now().strftime('%y_%m_%d_%H_%M_%S') + '/'
...
def eval_leave_one_participant_out_group_each(config, user_id):
    raw_dataset_path = '../svm_model/timecare~25k_raw_filtered.csv'
    dataset_all = pd.read_csv(raw_dataset_path)
    group_dict = _get_user2group_dict(dataset_all)
    group_name = group_dict[user_id]
    group_all = _get_group_id(dataset_all)
    group_all.sort()
    user_all = _get_user_id(dataset_all)
    user_all.sort()
...
def eval_leave_one_participant_out_group_tune(config, user_id):
    raw_dataset_path = '../svm_model/timecare~25k_raw_filtered.csv'
    dataset_all = pd.read_csv(raw_dataset_path)
    group_dict = _get_user2group_dict(dataset_all)
    group_name = group_dict[user_id]
    ...
    save_list([['user_id','test_mape','test_fail_rate']], 'rl_lopo_metrics.csv')
...
def eval_general_predict(config):
    config['select_type'] = 'all'
    config['train_dataset_path'] = '../svm_model/timecare~25k_estimate_train_general_encode_norm.csv'
    train_dataset_all = pd.read_csv(config['train_dataset_path'])
    ...
    estimated_dataset_train = pd.DataFrame(columns = new_head)
    estimated_dataset_test = pd.DataFrame(columns = new_head)
    model_metrics_all = pd.DataFrame(columns = ['general','test_accu','test_f1','test_precision','test_recall','test_mae','test_mape','test_fail_rate'])
    model_metrics_all_svm = pd.DataFrame(columns = ['general','test_accu','test_f1','test_precision','test_recall','test_mae','test_mape','test_fail_rate'])
    
    general_model_root = 'general_model_test/'
    general_model_all_path = os.listdir(general_model_root)
    general_model_all_path.sort()
    config['log_dir'] = general_model_root + '/' + general_model_all_path[-1] + '/'

    config['env_type'] = 'test'
    config['test_format'] = 'all'
    config['test_dataset_path'] = 'test.csv'
    config['test_result_name'] = 'test_one_sample'
    config['dataset_all_path'] = config['test_dataset_path']
    test_dataset_all = pd.read_csv(config['test_dataset_path'])
    config['test_epi_num'] = _get_epi_num(test_dataset_all, config['select_type'], None, None, config['max_time_set'])
    data_set_test = _get_user_data(test_dataset_all, config['select_type'], None, None, config['max_time_set'])
    ...
    model_metrics_all.loc[0] = ['general'] + test_metric
    model_metrics_all_svm.loc[0] = ['general'] + test_metric_svm

    dataset_test_piece = pd.DataFrame(np.concatenate((np.array(data_set_test)[:,:-3],answer_test_estimated,resptime_test_estimated),axis=1), columns=new_head)
    estimated_dataset_test = pd.concat([estimated_dataset_test, dataset_test_piece],ignore_index=True)
    
    estimated_dataset_test.to_csv('DRL_DDM_general_test_sample.csv',index=False)
    model_metrics_all.to_csv('DRL_DDM_general_test_metrics.csv',index=False)
    model_metrics_all_svm.to_csv('svm_general_test_metrics.csv',index=False)
    


stimuli_discounter_dict = {1: 0.0008, 2: 0.0001, 3: 0.01, 4: 0.0001, 5: 0.02, 6: 0.005, 7: 0.005, 8: 0.0008, 10: 0.0003, 11: 0.005, 
                            13: 0.0005, 14: 0.005, 15: 0.001, 16: 0.005, 17: 0.01, 18: 0.006, 19: 0.0003, 20: 0.01, 21: 0.0005, 
                            22: 0.01, 23: 0.0001, 24: 0.005, 25: 0.006, 26: 0.001, 27: 0.003, 28: 0.0008, 29: 0.008, 30: 0.005,
                            31: 0.0005, 32: 0.005, 33: 0.005, 34: 0.0001, 35: 0.0005, 36: 0.001, 40: 0.001, 41: 0.015}
stimuli_discounter_group_dict = {'none': 0.012, 'static': 0.003, 'random': 0.01, 'rule': 0.01}
stimuli_discounter_general = 0.012

stimuli_discounter_lopo_dict = {1: 0.01, 2: 0.01, 3: 0.01, 4: 0.0001, 5: 0.01, 6: 0.01, 7: 0.01, 8: 0.01, 10: 0.01, 11: 0.0055, 
                            13: 0.0005, 14: 0.0001, 15: 0.012, 16: 0.01, 17: 0.01, 18: 0.0045, 19: 0.012, 20: 0.01, 21: 0.0005, 
                            22: 0.01, 23: 0.0008, 24: 0.01, 25: 0.01, 26: 0.01, 27: 0.01, 28: 0.0008, 29: 0.008, 30: 0.005,
                            31: 0.0001, 32: 0.0012, 33: 0.0008, 34: 0.01, 35: 0.01, 36: 0.0012, 40: 0.003, 41: 0.01}
...
if args.type == 'eu':
    ...
    for group_name in stimuli_discounter_group_dict.keys():
        config['stimuli_discounter'] = stimuli_discounter_group_dict[group_name]
        eval_group_each(config, group_name) 
elif args.type == 'ea':
    config['stimuli_discounter'] = 0.012
    eval_general_all(config)
elif args.type == 'eah':
    stimuli_discounter_list = [0.015, 0.012, 0.02]
    for stimuli_discounter in stimuli_discounter_list:
        config['stimuli_discounter'] = stimuli_discounter
        eval_general_tune(config)
elif args.type == 'el':
    eval_leave_one_participant_out_group(config)
elif args.type == 'elh':
    for user_id in stimuli_discounter_lopo_dict.keys():
        print(user_id, stimuli_discounter_lopo_dict[user_id])
        config['stimuli_discounter'] = stimuli_discounter_lopo_dict[user_id]
        eval_leave_one_participant_out_group_each(config,user_id)

elif args.type == 'eh':
    user_id_list = [] 
    for user_id in stimuli_discounter_dict.keys():
        print(user_id, stimuli_discounter_dict[user_id])
        config['stimuli_discounter'] = stimuli_discounter_dict[user_id]
        eval_user_each(config, user_id)
elif args.type == 'ep':
    eval_general_predict(config)
elif args.type == 'abf':
